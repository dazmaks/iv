...
impl super::ImageBoard for Fourchan {
	...
	fn get_threads(&self, board: Board) -> Vec<ThreadInfo> {
		...
		let threads: Vec<ThreadInfo> = deserialized.threads.into_iter().map(|t|
			ThreadInfo {
				thread: Thread {board: board.clone(), id: t.posts[0].no.to_string()},
				comment: parse_html(&t.posts[0].com.clone().unwrap_or("unknown".to_owned())),
				posts_count: t.posts[0].replies.unwrap_or(0),
				timestamp: t.posts[0].time
			}).collect();
		threads
	}
	...
	fn get_thread_posts(&self, _thread: Thread) -> Vec<Post> {
		vec![
			Post {
				id: "id".to_owned(),
